...
sgk_wordwrap/
  ...
  gui/
    tray.py           - SgkTrayIcon: PyQt6 tray, left-click toggle, themed menu, About dialog
    config_dialog.py  - SgkConfigDialog: tabbed settings (General/Hotkeys/Blacklist/Behavior)
    i18n.py           - sgk_tr(): EN/RU string table for the tray + settings
    icon.png / icon-mono.png - colour and monochrome tray icon variants
  utils/
    config.py         - SgkConfig: ~/.config/sgk-wordwrap/config.json, deep-merged defaults
    autostart.py      - sgk_set_autostart(): ~/.config/autostart/wordwrap.desktop
```
...
- **UI text is hyphen-only** (dash char `-`, never em/en dash). `test_i18n` guards
  the string table; keep it that way in code comments and docs too.
- Language and tray-icon style apply live via `tray.sgk_apply_ui()` (wired through
  `SgkConfigDialog(on_ui_changed=...)` -> `app._sgk_apply_ui`). Hotkeys still need a
  restart.
- Autostart source of truth is the file `~/.config/autostart/wordwrap.desktop`, not
  a config key; the Settings checkbox reads/writes it via `utils/autostart.py`.

### Config keys of note

`ui.language` ("en" | "ru"), `ui.tray_icon_style` ("color" | "mono"),
