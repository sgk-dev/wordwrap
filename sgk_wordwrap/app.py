...
class SgkApp:
    ...
    def sgk_start(self) -> None:
        ...
        _logger.info("sgk_app_starting", extra={"version": "0.1.0"})
        ...
        def _schedule_process() -> None:
            if self._loop:
                asyncio.run_coroutine_threadsafe(processor.sgk_process(), self._loop)
            ...
            def _toggle_pause() -> None:
                if self._hotkey_manager:
                    if self._hotkey_manager.sgk_is_paused():
                        ...
                        if self._tray:
                            self._tray.sgk_set_paused(False)
                    else:
                        self._hotkey_manager.sgk_pause()
                        if self._tray:
                            self._tray.sgk_set_paused(True)
